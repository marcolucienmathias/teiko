...
def indiv_cell_generator(x, cell_cols):
    for _, x in df.iterrows():

        cell_tot = rowsum(x)

        for cell_type in cell_cols:
            yield {'sample' : x['sample'], 'total_count' : cell_tot, 'population' : cell_type, 'count' : x[cell_type], 'percentage' : 100*round(x[cell_type]/cell_tot,2)}
...
df_stats.to_csv("clinical_trial_statistics.csv", index=False) #save
